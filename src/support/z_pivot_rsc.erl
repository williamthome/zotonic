...
%% Task queue max retries if task crashes
...
execute_task({TaskId, Module, Function, _Key, Args, RetryCt, Props}, Context) ->
    try
        ...
        error:undef ->
            lager:error("Undefined task, aborting: ~p:~p(~p) ~p~n",
                        [Module, Function, Args, erlang:get_stacktrace()]),
            z_db:q("delete from pivot_task_queue where id = $1", [TaskId], Context);
        Error:Reason ->
            lager:error("Task failed(~p:~p): ~p:~p(~p) retry:~p ~p~n", 
                        [Error, Reason, Module, Function, Args, RetryCt, erlang:get_stacktrace()]),
